...
class Yoda_Packs {
  ...
  public static function set_kako_cookie($kakoId, $ttl = self::COOKIE_TTL){
    ...
    if ($host && !preg_match('/^\d+\.\d+\.\d+\.\d+$/', $host) && $host !== 'localhost'){
      $parts = explode('.', $host);
      // monta candidatos do mais específico para o mais amplo (mínimo 2 labels)
    ...
    // 1) host atual (sem domínio explícito)
    @setcookie(self::COOKIE_KAKO_ID, $kakoId, $expire, $path, '', $secure, false);
    // 2) domínios candidatos (www, apex etc.)
    foreach (array_unique($cands) as $d){
      @setcookie(self::COOKIE_KAKO_ID, $kakoId, $expire, $path, $d, $secure, false);
    }
  }

  public function assets(){
    // só carrega nas páginas que tenham o shortcode
    if (!is_singular() && !is_front_page()) return;

    // CSS simples (cores e locks)
    $css = "
    ...
    // JS
    wp_enqueue_script('yoda-packs', plugins_url('../assets/yoda-packs.js', __FILE__), ['jquery'], '1.0', true);
  ...
  public function shortcode($atts){
    $atts = shortcode_atts([
      // opção: categoria dos produtos de moedas
      'category' => '', // se usar, ex: coins
    ], $atts);

    // Checa cookie (se já verificado)
    $prefill_id = isset($_COOKIE[self::COOKIE_KAKO_ID]) ? sanitize_text_field($_COOKIE[self::COOKIE_KAKO_ID]) : '';

    // Consulta produtos de moedas (critério: meta _yoda_coins_amount)
    $args = [
      'status'  => 'publish',
      'limit'   => 12,
      'orderby' => 'menu_order',
  ...
  public function ajax_verify(){
    ...
    if (is_wp_error($res)){
      ...
      if (stripos($msg, 'cURL error 28') !== false){
        // Tenta com transporte streams
        $filter = function($t){ return ['streams']; };
        add_filter('http_api_transports', $filter);
        try { $res = $client->userinfo($kakoId); }
        finally { remove_filter('http_api_transports', $filter); }
        if (is_wp_error($res)){
    ...
    // cookie por 24h (legível por JS e visível em subdomínios)
    self::set_kako_cookie($kakoId, self::COOKIE_TTL);
    set_transient(self::cache_key($kakoId), [
      'kakoId'   => $kakoId,
      'avatar'   => $avatar,
      'nickname' => $nickname,
      'openId'   => $openId,
